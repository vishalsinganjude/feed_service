...
from feed_service.db.feed_models.models import Upvote, Downvote
...
def create_upvote(request_data):
    ...
    try:
        downvote_object = Downvote.objects.filter(user_id=username, answer=answer_object).first()

        if downvote_object:
            downvote_object.delete()
